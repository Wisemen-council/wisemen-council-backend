...
internal class UserServiceTest {
    ...
    private val A_SIGN_UP_DATA = SignUpData(AN_EMAIL, A_PASSWORD, A_NAME)
    private val A_USER_NO_ID = mapSignUpDataToUserEntity(A_SIGN_UP_DATA)
    private val A_USER = UserEntity(1L, AN_EMAIL, A_PASSWORD, A_NAME)

    @Test
    fun createUser_callsRepository() {
        `when`(userRepository.save(A_USER_NO_ID)).thenReturn(A_USER)
        userService.createUser(A_SIGN_UP_DATA)

        Mockito.verify(userRepository).save(A_USER_NO_ID)
    }

    @Test
    fun createUser_whenCreationSucceeds_returnsUser() {
        `when`(userRepository.save(A_USER_NO_ID)).thenReturn(A_USER)
        val user = userService.createUser(A_SIGN_UP_DATA)

        assertThat(user).extracting("email", "password", "full_name")
                .containsExactly(A_USER.email, A_USER.password, A_USER.full_name)
    }

    @Test
    fun createUser_whenEmailIsInvalid_returnsSignUpException() {
        assertThrows<SignUpException> {
            val INVALID_SIGN_UP_DATA = SignUpData("invalid_email", A_PASSWORD, A_NAME)
    ...
    fun createUser_whenPasswordIsNotLongEnough_returnsSignUpException() {
        assertThrows<SignUpException> {
            val INVALID_SIGN_UP_DATA = SignUpData(AN_EMAIL, "", A_NAME)
            userService.createUser(INVALID_SIGN_UP_DATA)
        }
    }
}
